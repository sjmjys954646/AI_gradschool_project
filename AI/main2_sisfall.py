...
class PaperEvalConfig:
    ...
    seed: int = 42
    num_workers: int = 0
    save_dir: str = "./paper_eval_outputs"
    device: str = "cuda" if torch.cuda.is_available() else "cpu"

    # paper-like hold-out split
    test_size: float = 0.2

    # if True: participant-level hold-out
    # if False: plain sample-level hold-out
...
# ============================================================
# 1) Participant-level 0.8 / 0.2 hold-out
# ============================================================
def run_paper_holdout(cfg: PaperEvalConfig):
    set_seed(cfg.seed)
    ...
    print("=" * 80)
    print("HOLD-OUT SPLIT (paper-style)")
...
# ============================================================
def run_kfold_cv(cfg: PaperEvalConfig, n_splits: int = 5):
    set_seed(cfg.seed)
    os.makedirs(cfg.save_dir, exist_ok=True)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(acc, labels), start=1):
        print("=" * 80)
        ...
        _, fold_metrics = train_and_eval_one_split(
            ...
            x_acc_test, x_gyro_test, y_test,
            cfg,
            split_name=f"{n_splits}fold-{fold_idx}",
        )
    ...
    print("=" * 80)
    print(f"{n_splits}-FOLD FINAL SUMMARY")
...
if __name__ == "__main__":
    cfg = PaperEvalConfig(
        ...
        batch_size=128,
        lr=1e-3,
        epochs=20,   # 논문은 최대 300 iteration이라고 적었지만, 네 기존 코드 스타일 유지
        dropout=0.5,
        base_channels=64,
        seed=42,
        num_workers=0,
        save_dir="./paper_eval_outputs",
        device="cuda" if torch.cuda.is_available() else "cpu",
        test_size=0.2,
        use_group_holdout=True,   # participant-level 80/20
    )

    # 1) 논문식 0.8 / 0.2 participant split
    run_paper_holdout(cfg)

    # 2) 논문식 5-fold CV
    run_kfold_cv(cfg, n_splits=5)

    # 3) 논문식 10-fold CV
    run_kfold_cv(cfg, n_splits=10)