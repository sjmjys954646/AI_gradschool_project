...
def run_loso(cfg: LOSOConfig):
    ...
    for fold_idx, test_subject in enumerate(unique_subjects, start=1):
        ...
        num_fall = (y_test == 1).sum()

        if num_fall == 0:
            print(f"⚠️ Skip subject {test_subject} (no fall in test set)")
            continue

        print(f"Train samples: {len(y_train)} | Test samples: {len(y_test)}")
        print(f"Test subject {test_subject} - Fall: {(y_test == 1).sum()}, ADL: {(y_test == 0).sum()}")
        ...
        torch.save(
            ...
            os.path.join(cfg.save_dir, f"dscs_subject_{int(test_subject)}.pt"),
        )
