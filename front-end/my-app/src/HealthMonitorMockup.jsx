...
export default function HealthMonitorMockup() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      const res = await fetch("http://127.0.0.1:8000/alarm/latest?user_id=1");
      const result = await res.json();

      if (result.has_alarm) {
        setShowFallAlert(true);
        setCurrentAlarmId(result.alarm_id);
      }
    }, 1000);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleCloseFallAlert = async () => {
    // try {
    //   if (currentAlarmId) {
    //     await fetch(`http://127.0.0.1:8000/alarm/${currentAlarmId}/check`, {
    //       method: "POST",
    //     });
    //   }
    // } catch (err) {
    //   console.error("Alarm check API error:", err);
    // }
